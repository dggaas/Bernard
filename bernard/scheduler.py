...
async def scheduler_mark_work_done(work):
    now = int(time.time())
    logger.info("scheduler_mark_work_done() marking JOB ID {0} DONE AT {1}".format(work['id'], now))
    database.cursor.execute("UPDATE scheduled_tasks SET exec_run=1, exec_when=%s WHERE id=%s", (now, work['id']))
    database.connection.commit()

async def scheduler_post_message(work):
    now = int(time.time())
    logger.info("POST_MESSAGE JOB ID {0[id]} CALLED BY {0[id_invoker]} TO POST IN CHANNEL {0[channel_targeted]}".format(work))

    #build the message channel object to send in the message
    send_channel = discord.discord.Object(work['channel_targeted'])
    await discord.bot.send_message(send_channel, "<@{0[id_targeted]}> Asked for a reminder: {0[event_message]}".format(work))
...
async def scheduler_unban_member(work):
    now = int(time.time())
    logger.info("UNBAN_MEMBER JOB ID {0[id]} BANNED BY {0[id_invoker]} BAN HAS EXPIRED, UNBANNING {0[id_targeted]}".format(work))

    #call the unban
    unban_status_discord = await common.unban_id(str(work['id_targeted']))
    if unban_status_discord:
        logger.info("UNBAN_MEMBER JOB ID {0[id]} USER ID {0[id_targeted]} BAN SUCCESSFULLY REMOVED FROM DISCORD.".format(work))

        #update the modlog channel
        await discord.bot.send_message(discord.mod_channel(),"{0} **Scheduled Unban:** ID:`{1[id_targeted]}`)".format(common.bernardUTCTimeNow(), work))

        #set a user journal record the unban has happened
        journal.update_journal_event(module=__name__, event="ON_MEMBER_UNBAN_SCHEDULED", userid=work['id_targeted'], contents="SCHEDULER JOBID: {0[id]}".format(work))

    else:
        logger.warn("UNBAN_MEMBER JOB ID {0[id]} USER ID {0[id_targeted]} BAN WAS NOT REMOVED FROM DISCORD, USER ALREADY UNBANNED?".format(work))
